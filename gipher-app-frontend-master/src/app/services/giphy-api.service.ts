...
export class GiphyAPIService {
  ...
  fetchTrendingGifs(typeChoice: number) {
    if (typeChoice == 0) {
      this.httpClient.get(`${this.baseurl}/v1/gifs/trending?api_key=${this.apiKey}`).subscribe(
        data => {
      ...
      this.httpClient.get(`${this.baseurl}/v1/stickers/trending?api_key=${this.apiKey}`).subscribe(
        data => {
          this.trending = this.initializeAndPush(data);
        },
        error => console.log(error.message)
  ...
  fetchSearchedGif(searchString: string, typeChoice: number) {
    if (typeChoice == 0) {
      this.httpClient.get(`${this.baseurl}/v1/gifs/search?api_key=${this.apiKey}&q=${searchString}`).subscribe(
        data => this.searchedGifs = this.initializeAndPush(data),
        error => console.log(error.message)
      );
    } else if (typeChoice == 1) {
      this.httpClient.get(`${this.baseurl}/v1/stickers/search?api_key=${this.apiKey}&q=${searchString}`).subscribe(
        data => this.searchedGifs = this.initializeAndPush(data),
        error => console.log(error.message)
      );
    }
  ...
  fetchGifById(gifId: string): Observable<any> {
    return this.httpClient.get<any>(`${this.baseurl}/v1/gifs/${gifId}?api_key=${this.apiKey}`);
  }

  
  fetchAllFavourites() {
    this.httpClient.get<Array<Giffy>>(`${this.baseurl}/favourites/getAllFavourites`).subscribe(
  ...
  addToFavourites(gif: Giffy) {
    console.log(gif);
    this.httpClient.post<Giffy>(`${this.baseurl}/favourites/addToFavourites`, gif).subscribe(
      data => {
        console.log(data);
  ...
  deleteFromFavourites(gif: Giffy) {
    this.httpClient.delete(`${this.baseurl}/favourites/deleteFromFavourites/${gif.favId}`).subscribe(
      data => this.fetchGifByEmailId(gif.emailId),
      error => console.log(error.message)
    )
  }
  fetchGifByEmailId(emailId: string) {
    this.httpClient.get<any>(`${this.baseurl}/favourites/getAllFavouritesByEmailId/${emailId}`).subscribe(data => {
      console.log(data);
